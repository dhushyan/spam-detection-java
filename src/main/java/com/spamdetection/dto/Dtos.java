package com.spamdetection.dto;

import lombok.Data;
import jakarta.validation.constraints.NotBlank;
import jakarta.validation.constraints.Email;
import jakarta.validation.constraints.Size;

/**
 * DTOs — replaces Django's forms.py
 * These are the data objects passed between Controller ↔ Service ↔ Template
 */
public class Dtos {

    // ---- replaces Django EmailCheckForm ----
    @Data
    public static class EmailCheckRequest {
        @NotBlank(message = "Email content cannot be empty")
        private String content;
    }

    // ---- ML API response from Python Flask ----
    @Data
    public static class MlPredictionResponse {
        private String prediction;   // "spam" or "ham"
        private double confidence;   // 0.0 to 1.0
        private String message;
    }

    // ---- replaces Django UserCreationForm ----
    @Data
    public static class RegisterRequest {
        @NotBlank(message = "Username is required")
        @Size(min = 3, max = 50)
        private String username;

        @NotBlank(message = "Email is required")
        @Email(message = "Enter a valid email")
        private String email;

        @NotBlank(message = "Password is required")
        @Size(min = 6, message = "Password must be at least 6 characters")
        private String password;

        @NotBlank(message = "Please confirm your password")
        private String confirmPassword;
    }

    // ---- Dashboard statistics ----
    @Data
    public static class DashboardStats {
        private long totalChecked;
        private long spamCount;
        private long hamCount;
        private double spamPercentage;
    }
}
